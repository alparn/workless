...
const CATEGORY_KEYS: Record<string, string> = {
  cash_deposit: "category_cashDeposit",
  cash_withdrawal: "category_cashWithdrawal",
  owner_transfer: "category_ownerTransfer",
  vague_reference: "category_vagueReference",
  unknown_private_person: "category_unknownPrivatePerson",
  loan_indicator: "category_loanIndicator",
  large_unidentified: "category_largeUnidentified",
};
...
export default function ClarificationsPage() {
  ...
  const t = useTranslations("clarifications");
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      showToast(
        err instanceof ApiError ? err.detail : t("downloadFailed"),
        "error",
      );
    } finally {
      setPdfLoading(false);
    ...
    .map((g) => ({
      ...
      items: g.items.filter((item) => {
        ...
        return [
          ...
          getCategoryLabel(item.clarification_category),
          item.booking_text,
          item.amount,
          item.debit_credit,
    ...
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {data.open_count > 0 && (
              <Badge variant="destructive">{t("openCount", { count: data.open_count })}</Badge>
            )}
            {data.resolved_count > 0 && (
              <Badge variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-700">
                {t("resolvedCount", { count: data.resolved_count })}
              </Badge>
            )}
          </div>
      ...
      {data.total_count === 0 ? (
        <div className="mt-16 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-medium">{t("noOpenTitle")}</p>
        ...
        <div className="mt-6 flex flex-col gap-6">
          ...
          <div className="flex gap-1 rounded-lg bg-muted p-1">
            {(
              [
                { key: "open", label: t("filterOpen"), count: data.open_count },
                {
                  key: "resolved",
                  label: t("filterResolved"),
                  count: data.resolved_count,
                },
                { key: "all", label: t("filterAll"), count: data.total_count },
              ] as const
            ).map(({ key, label, count }) => (
              <button
                key={key}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    >
                      ...
                      {pdfLoading ? t("creating") : t("downloadPdf")}
                    </Button>
                  </div>
                </div>
              </CardHeader>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {t("uploaded")} {formatDate(group.uploaded_at)}
          </p>
        </div>
        <div className="flex shrink-0 gap-2">
          {group.open_count > 0 && (
            <Badge variant="destructive" className="text-xs">
              {t("openCount", { count: group.open_count })}
            </Badge>
          )}
          {group.resolved_count > 0 && (
            ...
            >
              {t("resolvedCount", { count: group.resolved_count })}
            </Badge>
          )}
        </div>
      </button>
...
function ClarificationRow({
  ...
  const categoryLabel = getCategoryLabel(item.clarification_category);
  const amountColor =
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setAnswerText("");
      showToast(t("clarificationAnswered"), "success");
    } catch (err) {
      showToast(
        err instanceof ApiError ? err.detail : common("saveFailed"),
        "error",
      );
    } finally {
      setSubmitting(false);
  ...
  const handleReopen = async () => {
    ...
    try {
      ...
      onUpdate(updated);
      showToast(t("clarificationReopened"), "success");
    } catch (err) {
      showToast(
        err instanceof ApiError ? err.detail : t("reopenError"),
        "error",
      );
    } finally {
      setSubmitting(false);
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.clarification_resolved && item.clarification_answer && (
          <div className="mt-2 flex gap-2 rounded-md bg-emerald-50 p-3">
            ...
            <div className="flex-1">
              <p className="text-xs font-medium text-emerald-700">{t("answer")}</p>
              <p className="mt-0.5 text-sm text-emerald-900">
                {item.clarification_answer}
              </p>
              {item.clarification_resolved_at && (
                <p className="mt-1 text-xs text-emerald-600">
                  {t("answeredOn")} {formatDate(item.clarification_resolved_at)}
                </p>
              )}
            </div>
            <Button
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  >
                    ...
                    {submitting ? common("saving") : t("submitAnswer")}
                  </Button>
                </div>
              </div>
            ) : (
