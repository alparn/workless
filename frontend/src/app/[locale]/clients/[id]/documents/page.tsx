...
export default function DocumentsPage() {
  ...
  const STATUS_LABELS: Record<string, { label: string; variant: "default" | "secondary" | "outline" | "destructive" }> = {
    uploaded:          { label: t("statusUploaded"),      variant: "outline" },
    ocr_processing:   { label: t("statusOcrRunning"),    variant: "secondary" },
    ocr_complete:     { label: t("statusOcrComplete"),   variant: "secondary" },
    ocr_failed:       { label: t("statusOcrFailed"),     variant: "destructive" },
    booking_suggested: { label: t("statusSuggestion"),   variant: "default" },
    booking_failed:   { label: t("statusBookingFailed"), variant: "destructive" },
    approved:         { label: t("statusApproved"),      variant: "default" },
    booked:           { label: t("statusBooked"),        variant: "default" },
    exported:         { label: t("statusExported"),      variant: "default" },
  };
  ...
  const handleDelete = async (docId: string) => {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : common("deleteFailed"),
        "error",
      );
    }
  };
  ...
  const filteredDocuments = documents.filter((doc) => {
    ...
    return [
      ...
      doc.status,
      doc.mime_type,
      new Date(doc.uploaded_at).toLocaleDateString(dateLocale),
    ]
      .filter(Boolean)
    ...
    <main className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
      ...
      {documents.length === 0 ? (
        <div className="mt-16 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-medium">{t("noDocumentsTitle")}</p>
