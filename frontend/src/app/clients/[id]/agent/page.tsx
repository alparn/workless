...
const SEVERITY_CONFIG: Record<string, { icon: typeof InfoIcon; color: string; label: string }> = {
  success: { icon: CheckCircle2Icon, color: "text-green-600", label: "Erfolg" },
  info: { icon: InfoIcon, color: "text-blue-600", label: "Info" },
  warning: { icon: AlertTriangleIcon, color: "text-amber-600", label: "Warnung" },
  error: { icon: AlertCircleIcon, color: "text-red-600", label: "Fehler" },
};

const CATEGORY_LABELS: Record<string, string> = {
  ocr_self_healing: "OCR-Wiederherstellung",
  booking_validation: "Buchungsprüfung",
  stale_detection: "Timeout-Erkennung",
  escalation: "Eskalation",
};

const RUN_TYPE_LABELS: Record<string, { label: string; icon: typeof ZapIcon }> = {
  ocr_self_healing: { label: "OCR-Wiederherstellung", icon: FileSearchIcon },
  booking_validation: { label: "Buchungsprüfung", icon: ShieldCheckIcon },
};
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {client?.company_name} — Automatische Qualitätssicherung
            </p>
          </div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-4 gap-4">
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">
                {agentStatus?.supervisor_enabled ? "Aktiv" : "Deaktiviert"}
              </span>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Prüft alle {agentStatus?.supervisor_interval_seconds}s
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Ungelesene Nachrichten</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{counts?.unread ?? 0}</p>
            <p className="text-xs text-muted-foreground">
              von {counts?.total ?? 0} gesamt
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Aktion erforderlich</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Prüfläufe</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Max. {agentStatus?.max_ocr_retries} OCR-Retries
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            <Card>
              <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
                ...
                <p className="text-muted-foreground">
                  Keine Nachrichten vom Prüfer — alles in Ordnung.
                </p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {n.entity_type && n.entity_id && (
                          <div className="mt-2">
                            ...
                            >
                              {n.entity_type === "document" ? "Zum Dokument →" : "Zur Buchung →"}
                            </Link>
                          </div>
                        )}
                      </div>
                        ...
                        <div className="mt-1 flex items-center gap-4 text-xs text-muted-foreground">
                          ...
                          {run.items_fixed > 0 && (
                            <span className="text-green-600">
                              {run.items_fixed} behoben
                            </span>
                          )}
                          {run.items_flagged > 0 && (
                            <span className="text-amber-600">
                              {run.items_flagged} markiert
                            </span>
                          )}
                        </div>
                      </div>
